import { GoogleGenerativeAI } from '@google/generative-ai';
import { Player } from '@masquerade/shared';
import * as dotenv from 'dotenv';
import * as path from 'path';

// Explicitly load .env from the bot root directory just to be bulletproof
dotenv.config({ path: path.join(__dirname, '..', '..', 'bot', '.env') });
dotenv.config({ path: path.join(__dirname, '..', '.env') });

let model: any = null;

function getModel() {
  if (!model) {
    const apiKey = process.env.GEMINI_API_KEY || '';
    if (!apiKey) return null;
    const genAI = new GoogleGenerativeAI(apiKey);
    model = genAI.getGenerativeModel({ model: 'gemini-3.1-flash-lite' });
  }
  return model;
}

/**
 * Sanitizes the LLM output by trimming whitespace, newlines, and trailing punctuation.
 */
function sanitizeOutput(text: string): string {
  return text.trim().replace(/[.,!?]+$/, '').replace(/\n/g, '');
}

export class LLMClient {
  private static async generateWithRetry(prompt: string, maxRetries = 3): Promise<string> {
    const aiModel = getModel();
    if (!aiModel) throw new Error("Model not initialized");

    for (let i = 0; i < maxRetries; i++) {
      try {
        const result = await aiModel.generateContent(prompt);
        return result.response.text();
      } catch (err) {
        console.error(`LLM generation attempt ${i + 1} failed:`, err);
        if (i === maxRetries - 1) throw err;
        // Wait before retrying (exponential backoff: 1s, 2s, 4s...)
        await new Promise(res => setTimeout(res, 1000 * Math.pow(2, i)));
      }
    }
    return "";
  }

  static async generateClue(
    role: string,
    secretWord: string,
    wordHistory: string[]
  ): Promise<string> {
    const aiModel = getModel();
    if (!aiModel) {
      console.warn("GEMINI_API_KEY not set! Falling back to default clue.");
      return "hmm";
    }

    const historyStr = wordHistory.length > 0 ? wordHistory.join(", ") : "None yet";

    let prompt = "";
    if (role === 'Impostor') {
      prompt = `You are playing Masquerade. You are the IMPOSTOR.
You do NOT know the secret word.
Previous clues given by players: ${historyStr}.
Provide a single-word clue that blends in with the others and makes sense given the context.
Constraint: Respond with EXACTLY one word. No punctuation, no markdown, no extra text.`;
    } else {
      prompt = `You are playing Masquerade. You are an INNOCENT.
The secret word is '${secretWord}'.
Previous clues given by players: ${historyStr}.
Provide a single-word clue that proves you know the word without making it too obvious for the Impostor.
Constraint: Respond with EXACTLY one word. No punctuation, no markdown, no extra text.`;
    }

    try {
      const responseText = await this.generateWithRetry(prompt);
      return sanitizeOutput(responseText);
    } catch (err) {
      console.error("Error generating clue after retries:", err);
      // Fallback word if all retries fail so the game doesn't look broken
      return "hmm";
    }
  }

  static async generateVote(
    role: string,
    players: Player[],
    wordHistory: { playerId: string; word: string }[]
  ): Promise<string> {
    const aiModel = getModel();
    if (!aiModel) {
      console.warn("GEMINI_API_KEY not set! Falling back to first player's vote.");
      return players[0]?.id || "";
    }

    const playerList = players.map(p => `- ${p.name} (ID: ${p.id})`).join("\n");
    const cluesList = wordHistory.map(entry => {
      const p = players.find(p => p.id === entry.playerId);
      return `${p?.name || entry.playerId}: ${entry.word}`;
    }).join("\n");

    let prompt = "";
    if (role === 'Impostor') {
      prompt = `You are playing Masquerade. You are the IMPOSTOR.
The other players are trying to vote you out.
Analyze the clues:
${cluesList}

Pick one Innocent player ID from this list to vote for to deflect suspicion and blend in:
${playerList}

Constraint: Output EXACTLY their UUID and nothing else. No markdown, no extra text.`;
    } else {
      prompt = `You are playing Masquerade. It is time to vote for the Impostor.
Players:
${playerList}

Clues given:
${cluesList}

Analyze the clues to find the player who gave a suspicious or disjointed clue.
Constraint: Output EXACTLY the UUID of the player you are voting for and nothing else. No markdown, no extra text.`;
    }

    try {
      const responseText = await this.generateWithRetry(prompt);
      return sanitizeOutput(responseText);
    } catch (err) {
      console.error("Error generating vote after retries:", err);
      return players[0]?.id || "";
    }
  }
}
